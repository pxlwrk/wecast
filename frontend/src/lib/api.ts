...
export const api = {
  ...
  videos: {
    ...
    triggerProcessing: (id: number) =>
      request(`/videos/${id}/process`, { method: "POST" }),
    update: (id: number, body: Partial<{ title: string; description: string; status: string }>) =>
  ...
  recordings: {
    start: (title: string, description?: string) =>
      request<import("@/types").VideoDetail>(`/recordings/start?title=${encodeURIComponent(title)}${description ? `&description=${encodeURIComponent(description)}` : ""}`, { method: "POST" }),
